...
post = Table('post', pre_meta,
    ...
    Column('created_date', VARCHAR(length=25)),
    Column('updated_date', VARCHAR(length=25)),
)
...
def upgrade(migrate_engine):
    ...
    pre_meta.tables['post'].columns['created_date'].drop()
    pre_meta.tables['post'].columns['updated_date'].drop()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    pre_meta.tables['post'].columns['created_date'].create()
    pre_meta.tables['post'].columns['updated_date'].create()
